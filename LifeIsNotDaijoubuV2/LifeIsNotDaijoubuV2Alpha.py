#On ne se bat pas dans l'espoir du succes! Non, non ! C'est bien plus beau lorsque c'est inutile !
# 
#jeu de plateforme
#Nom: 'LifeIsNotDaijoubu' ou 'Mr.Filiforme / LaLongueur court vers la sortie'
...
class Jeu:
	def __init__(self):
		self.tk = Tk()
		self.tk.title("LifeIsNotDaijoubou Early Access")
		...
		self.ap = PhotoImage(file='background.gif')
		larg = self.ap.width()
		haut = self.ap.height()
		for x in range(0, 5):
			for y in range(0, 5):
...
"""< BOF  BOF  Partie à optimiser..."""

					# Detecter Contact horizontal
def dans_x(co1, co2):
	if (co1.x1 > co2.x1 and co1.x1 < co2.x2):
		return True
	elif (co1.x1 > co2.x1 and co1.x2 < co2.x2):
		return True
	elif (co1.x1 > co2.x1 and co1.x1 < co2.x2):
		return True
	elif (co1.x2 > co2.x1 and co1.x2 < co2.x1):
		return True
	else:
		return False

					# Detecter Contact vertical 
def dans_y(co1, co2):
	if (co1.y1 > co2.y1 and co1.y1 < co2.y2):
		return True
	elif (co1.y1 > co2.y1 and co1.y2 < co2.y2):
		return True
	elif (co1.y1 > co2.y1 and co1.y1 < co2.y2):
		return True
	elif (co1.y2 > co2.y1 and co1.y2 < co2.y1):
		return True
	else:
		return False
"""... à optimiser prout prout >"""

					# Detecter collision à gauche 
...
def collision_haut(co1, co2):
	if dans_x(co1, co2):
		...
		return False 
					...
					# Detecter collision en bas (INCHALLUX CA PASSE)(╯°□°）╯︵ ┻━┻ lal la al ala la aa aala al
...
class LutinPersonnage(Lutin):
	def __init__(self, jeu):
		Lutin.__init__(self, jeu)
		self.images_gauche = [
			PhotoImage(file='boug-G1.gif'),
			PhotoImage(file='boug-G2.gif'),
			PhotoImage(file='boug-G3.gif')
		]
		self.images_droite = [
			PhotoImage(file='boug-D1.gif'),
			PhotoImage(file='boug-D2.gif'),
			PhotoImage(file='boug-D3.gif')
		]
...
class LutinPorte(Lutin):
	def __init__(self, jeu, image_photo, x, y, largeur, hauteur):
		Lutin.__init__(self, jeu)
		self.image_photo = image_photo
		self.image_photo = jeu.canvas.create_image(x, y, image=self.image_photo, anchor='nw')
		self.coordonnees = Coords(x, y, x + (largeur/2 ), y + hauteur)
		self.finjeu = True

jeu = Jeu()

plateforme1  = LutinPlateForme(jeu, PhotoImage(file='plate-formea.gif'), 0  , 480, 100, 10)
plateforme2  = LutinPlateForme(jeu, PhotoImage(file='plate-formea.gif'), 150, 440, 100, 10)
plateforme3  = LutinPlateForme(jeu, PhotoImage(file='plate-formea.gif'), 300, 400, 100, 10)
plateforme4  = LutinPlateForme(jeu, PhotoImage(file='plate-formea.gif'), 300, 160, 100, 10)

plateforme5  = LutinPlateForme(jeu, PhotoImage(file='plate-formeb.gif'), 175, 350, 66 , 10)
plateforme6  = LutinPlateForme(jeu, PhotoImage(file='plate-formeb.gif'), 50 , 300, 66 , 10)
plateforme7  = LutinPlateForme(jeu, PhotoImage(file='plate-formeb.gif'), 170, 120, 66 , 10)
plateforme8  = LutinPlateForme(jeu, PhotoImage(file='plate-formeb.gif'), 45 , 60 , 66 , 10)

plateforme9  = LutinPlateForme(jeu, PhotoImage(file='plate-formec.gif'), 170, 250, 32 , 10)
plateforme10 = LutinPlateForme(jeu, PhotoImage(file='plate-formec.gif'), 230, 200, 32 , 10)
